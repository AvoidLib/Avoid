...
import pl.olafcio.avoid_impl.mixin.accessors.IFlowingFluid;
...
public final class NativeFluid extends Fluid {
    ...
    public boolean animateTick(World world, BlockPos blockPos, FluidState fluidState, RandomProvider randomProvider) {
        ((IFluid) fluid).avoid$animateTick(WorldNative.convert(world), BlockPosNative.convertFrom(blockPos), getState(fluidState), RandomProviderNative.convert(randomProvider));
        return OK;
    }
    ...
    public boolean tick(World world, BlockPos blockPos, FluidState fluidState, BlockData blockData) {
        ((IFluid) fluid).avoid$tick((ServerLevel) WorldNative.convert(world), BlockPosNative.convertFrom(blockPos), BlockDataNative.convert(blockData), getState(fluidState));
        return OK;
    }

    @Override
    public boolean randomTick(World world, BlockPos blockPos, FluidState fluidState, RandomProvider randomProvider) {
        ((IFluid) fluid).avoid$randomTick((ServerLevel) WorldNative.convert(world), BlockPosNative.convertFrom(blockPos), getState(fluidState), RandomProviderNative.convert(randomProvider));
        return OK;
    }
    ...
    public int getSpread(FluidState state) {
        return fluid.getAmount(getState(state));
    }
    ...
    private static net.minecraft.world.level.material.Fluid getFluid(Fluid fluid) {
        return Reflect.get(Fluid.class, "fluid", net.minecraft.world.level.material.Fluid.class, fluid);
    }

    private static net.minecraft.world.level.material.FluidState getState(FluidState state) {
        return Reflect.get(FluidState.class, "state", net.minecraft.world.level.material.FluidState.class, state);
    }

    @Override
    public BlockData createBlock(FluidState state) {
        return BlockDataNative.convertFrom(((IFluid) fluid).avoid$createLegacyBlock(getState(state)));
    }
}
